...
impl<S: 'static, B> Transform<S> for EnforceAuth
where
    ...
    fn call(
        ...
        Box::pin(async move {
            let mut db = match db.await {
                Ok(db) => db,
                Err(err) => { return Ok(req.into_response(HttpResponse::InternalServerError().finish().into_body())); }
            };

            let auth = Auth::from_sreq(&mut req, &mut db).await;
            ...
            let mut auth = match auth {
                Err(err) => {
                    if err.invalidate {
                        ...
                        {
                            return Ok(req.into_response(HttpResponse::Found().cookie(cookie).header(http::header::LOCATION, "/admin/login/index.html").finish().into_body()));
                        }
                    }
                ...
                || self.auth == ServerAuthDefault::Admin && auth.access == AuthAccess::Admin
            {
                if path.len() >= 1 && path[0] == String::from("admin") {
                    ...
                    if path.len() >= 2 && path[1] == String::from("login") {
                        ...
                        return Ok(req.into_response(HttpResponse::Found().header(http::header::LOCATION, "/admin/login/index.html").finish().into_body()));
                    }
                } else {
